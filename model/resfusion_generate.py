...
class GaussianResfusion_Generate(pl.LightningModule):
    """
    ...
    def __init__(self, denoising_module: pl.LightningModule, variance_scheduler: Scheduler,
        ...
        self.val_FID = FrechetInceptionDistance(normalize=True).set_dtype(torch.float64)
        self.test_FID = FrechetInceptionDistance(normalize=True).set_dtype(torch.float64)
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        X_0 = X_0 * 2 - 1
        # for image generation X_0_hat is zero
        X_0_hat = torch.zeros_like(X_0)
        ...
        if self.mode == 'epsilon':
            ...
            pred_resnoise = self.denoising_module(x=x_t, time=t)
            # Loss
            ...
            pred_x_0 = self.denoising_module(x=x_t, time=t)
            # Loss
            if self.loss_type == 'L2':
    ...
    def validation_step(self, batch, batch_idx: int):
        if batch['true_images'] is not None:
            # initialize X_0
            X_0 = batch['true_images']
            # calculate FID
            self.val_FID.update(X_0, real=True)
            # log images
            if batch_idx < 16:
                self.log_tb_images((X_0,), batch_idx, 0, tag="GroundTruth", save_all=False)
        if batch['pred_images'] is not None:
            # initialize X_0_hat
            X_0_hat = torch.zeros_like(batch['pred_images'])
            ...
            # calculate FID
            self.val_FID.update(pred_x_0, real=False)
            # log images
            if batch_idx < 16:
                self.log_tb_images((pred_x_0,), batch_idx, self.current_epoch, tag="Prediction", save_all=False)
        ...
        self.log('val_FID', self.val_FID, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
    ...
    def test_step(self, batch, batch_idx: int):
        if batch['true_images'] is not None:
            ...
            X_0 = batch['true_images']
            # calculate FID
            self.test_FID.update(X_0, real=True)
            # log images
            self.log_tb_images((X_0,), batch_idx, 0, tag="GroundTruth", save_all=True)
        if batch['pred_images'] is not None:
            # initialize X_0_hat
            X_0_hat = torch.zeros_like(batch['pred_images'])
            pred_x_0 = self.generate(X_0_hat)
            ...
            pred_x_0 = (pred_x_0 + 1) / 2
            # calculate FID
            self.test_FID.update(pred_x_0, real=False)
            # log images
            self.log_tb_images((pred_x_0,), batch_idx, self.current_epoch, tag="Prediction", save_all=True)
        # sync_dist, sync_dist_group and reduce_fx flags from self.log(...) don’t affect the metric logging
        # in any manner. The metric class contains its own distributed synchronization logic.
        self.log('test_FID', self.test_FID, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
    ...
    def generate(self, X_0_hat: torch.Tensor,
        ...
        :param X_0_hat: zeros like batch
        ...
        for t in range(self.T_acc - 1, -1, -1):
            ...
            if self.mode == 'epsilon':
                pred_resnoise = self.denoising_module(x=X_noise, time=t_tensor)  # predict the resnoise
                if t == 0:
                    z.fill_(0)
                ...
                pred_x_0 = self.denoising_module(x=X_noise, time=t_tensor)  # predict the x_0
                # threshold clip
                pred_x_0 = torch.clamp(pred_x_0, min=-1, max=1)
                if t == 0:
                    X_noise = pred_x_0
    ...
    def log_tb_images(self, viz_batch, batch_idx, current_epoch, tag, save_all=False) -> None:
        ...
        for img_idx, (image,) in enumerate(zip(*viz_batch)):
            if save_all:
                tb_logger.add_image(f"{tag}/{batch_idx}_{img_idx}", image, current_epoch)
            else:
                if img_idx < 16:
                    tb_logger.add_image(f"{tag}/{batch_idx}_{img_idx}", image, current_epoch)
